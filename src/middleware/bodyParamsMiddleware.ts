...
const bodyFieldValidationMiddleware = (req: Request, res: Response, next: NextFunction) => {
    ...
    console.debug(`Request received - Method: ${method}, Path: ${routePath}, Role: ${userRole}`);
    ...
    const endpointKey = Object.keys(AuthConfig.endpoints).find((key) => {
        ...
        return `${baseUrl}${routePath}` == endpoint.path && endpoint.method === method;
    });
    ...
    if (req.method === 'POST' || req.method === 'PUT') {
        ...
        if ('validateSchema' in endpointConfig && endpointConfig.validateSchema) {
            validatePayload(endpointConfig.validateSchema as ZodType, requestBody);
        }
